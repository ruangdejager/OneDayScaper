...
async def _send_item(context, user_id: int, text: str, image_url: str) -> None:
    """Send a single item — as photo+caption if image available, else text."""
    if image_url:
        try:
...
async def send_daily_notifications(context: CallbackContext) -> None:
    ...
    # ── Match OneDayOnly products per user ─────────────────────────────────────
    odo_matches = match_products(odo_products, subscriptions)

    # ── Scrape custom sites per user (deduplicated by URL) ─────────────────────
    # Cache results so the same URL isn't fetched multiple times
    ...
    for user_id, sites in all_user_sites.items():
        ...
        for site in sites:
            url = site["url"]
            cache_key = f"{url}|{'|'.join(sorted(keywords))}"
            if cache_key not in site_cache:
                site_cache[cache_key] = await asyncio.to_thread(scrape_site, url, keywords)
            ...
            if total > 0:
                ...
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"across <b>{source_str}</b> matching your keywords today:"
                    ),
                    parse_mode=ParseMode.HTML,
                )
                ...
                for result in matched_sites:
                    ...
                    await _send_item(
                        context, user_id,
                        format_generic_message(result),
                        result.image_url,
                    )
            else:
                kw_list = ", ".join(f"<i>{k}</i>" for k in info["keywords"])
                site_count = len(all_user_sites.get(user_id, []))
                sources_str = f"OneDayOnly + {site_count} custom site{'s' if site_count != 1 else ''}" if site_count else "OneDayOnly"
                await context.bot.send_message(
                    chat_id=user_id,
